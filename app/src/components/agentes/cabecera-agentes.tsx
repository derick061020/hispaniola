...
export function CabeceraAgentes() {
  return (
    <div>
      <Etiqueta sobreOscuro>{t('Travel agents')}</Etiqueta>

      <h1 className="mt-3 max-w-2xl text-balance font-display text-h2 font-semibold text-white">
        {t('Register your agency or DMC')}
      </h1>

      <p className="mt-4 max-w-2xl text-lead text-white/85">
        {t('We coordinate with you directly: real availability, formal invoicing (local and international) and answers on WhatsApp, not a call center.')}
      </p>
    </div>
  )
}
