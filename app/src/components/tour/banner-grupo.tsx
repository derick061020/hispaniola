  ...
  return (
    ...
    >
      ...
      <p className="relative z-[2] text-xs leading-relaxed">
        {conseguido ? (
          <>
            <strong className="font-semibold">{t('Group rate unlocked.')}</strong>{' '}
            {pct === null
              ? tp('Your group of {n} gets a group discount. We apply it when we confirm your booking.', {
                  n: personas,
                })
              : tp('Your group of {n} saves {pct}% on this booking.', { n: personas, pct })}
          </>
        ) : (
          <>
            ...
            {fueraDeAlcance ? (
              ...
              contactoUrl ? (
                ...
                >
                  {t('Message us to book a larger group.')}
                </a>
              ) : (
                <>{t('Message us to book a larger group.')}</>
              )
            ...
            faltan <= 3 ? (
              <>
                {faltan === 1
                  ? t('You’re 1 guest away.')
                  : tp('You’re {n} guests away.', { n: faltan })}
              </>
            ) : (
              <>{t('Bring the whole crew.')}</>
            )}
          </>
        )}
      </p>
