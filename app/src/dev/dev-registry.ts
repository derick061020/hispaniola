      ...
      },
      {
        ...
        states: [
          ...
          },
          {
            ...
            note: 'Como =estatico pero además pausa el video del océano en su propio poster (incluye-oceano-poster.webp) → frame 100% quieto para Figma (a Figma no va video).',
          },
        ],
      },
      {
        ...
        route: '/',
        status: 'done',
        description: 'Rediseño editorial (Pedro, 2026-07-16) — antes era una grid 2x2 de texto plano enterrada entre WhyDirect (blanca) y IncluyeCrucero (oscura). Ahora: spread editorial asimétrico en bg-papel-hueso (diferencia visual de las dos secciones vecinas). Layout lg:grid-cols-[2fr_3fr] + items-stretch: la foto de coral (galeria-snorkel-lovers-6) ocupa el ~40% izquierdo y se estira (h-full en el img) a la altura de las 4 verdades de la derecha — bloque unificado, no foto pequeña al lado de texto. A la derecha: header (eyebrow + h2 «No es otro party boat») y 4 <li> numerados (01, 02, 03, 04) con un NÚMERO ENORME de fondo (text-[8rem] / lg:text-[9rem], font-serif, text-navy/8, absolute top-left, pointer-events-none + select-none + aria-hidden) que crea ritmo vertical — la "marca tipográfica" de la sección. Cada <li> con icono lucide (Sprout/01 coral, ChefHat/02 cocina, Users/03 aforo, Leaf/04 eco) en text-aqua size-6 strokeWidth=1.5, título bold navy, descripción corta navy-sub leading-relaxed. Hover de GRUPO en cada <li>: el número de fondo sube de /8 a /15 y el icono escala 1.1× — feedback sutil que recompensa exploración. Los 4 datos son locales al componente (contenido de sección específico, no reutilizable). En el traspaso a Figma cada "verdad" es una instancia de un componente `Diferenciador/Item` con la prop Icon como slot (los lucide se convierten en `Icon/Sprout`, `Icon/ChefHat`, etc. por convención del proyecto).',
        states: [],
      },
      {
        title: 'Reviews',
