    ...
    screens: [
      {
        ...
        states: [
          ...
          },
          {
            ...
            kind: 'overlay',
            to: '/tours/semi-privado?dev-hero-galeria=abierta',
            note: 'T-F2, ahora disparado desde el mosaico incrustado en el hero (internas/galeria-hero.tsx): el lightbox con la galería completa del tour (portada + galeriaCompleta). Hereda los 4 arreglos de UX de la hoja del menú móvil (scroll del fondo bloqueado guardando el valor previo, foco que entra y vuelve al disparador, Escape, click fuera) porque el problema es el mismo: un overlay modal sobre la página. Extra propio de galería: ← → pasan foto. Isla Saona NO tiene lightbox — no hay galería suya y no se rellena con fotos de otros tours.',
          },
          {
    ...
    screens: [
      {
        ...
        status: 'done',
        description:
          'UNA plantilla data-driven para las 2 ocasiones con landing propia (Bodas y Empresas/MICE — las `esLanding: true` de OCASIONES; las otras 4 van al formulario del hub, que sigue en el prototipo). En Figma es una página con frames de variante, no 2 diseños. Copy portado verbatim de renderBodas/renderEmpresas del prototipo (data/eventos.ts). Anatomía: HERO compartido con la home y la ficha de tour (PLAN-INTERNAS-V2.md §C5, internas/hero-interna.tsx) — mismo box redondeado + Header `sobreVideo`, COMPACTO, con el video de marca (el mismo de la home) y el mosaico de fotos reales de charter-privado (`evento.galeria`) incrustado a su lado en sm+ (internas/galeria-hero.tsx, sin quote propia — FichaEvento no la tiene) y en tira al pie en móvil, con su propio lightbox (iteración 2026-07-17, misma pieza que la ficha de tour); dentro, cabecera en blanco (migaja, eyebrow, H1 en --text-h2, lead, fila de confianza, CTAs) · banda de cifras (solo empresas: pax/multi-barco/factura/seguro), ya en blanco, debajo del hero · formatos (3 cards foto+texto) · beneficios (mismo anatómico que IncluyeTour) + testimonio · banda de cierre navy (ancla del «Reservar» del header, #evento-cierre). Es página de PERSUASIÓN, no de conversión directa: el evento se cotiza (sin precio, sin widget, sin barra móvil) — el CTA de cotización es la frontera con el prototipo (EnlacePrototipo), igual que el funnel en la ficha. Lo que NO se pinta y por qué: la galería «Bodas reales» del prototipo (la única foto de boda real es la de la cabecera; rellenar con fotos de otros tours mentiría — mismo criterio que la galería vacía de Isla Saona) y los 6 logos «Han navegado con nosotros» de empresas (no hay logos reales de clientes). Fotos PROVISIONALES de la galería del charter privado, elegidas mirándolas (también las del hero); pendiente shooting de eventos (PLAN-v3.md §9). La home enlaza las 2 landings desde el megamenú de Eventos, el ticker del hero y el menú móvil (unión por `slug` en OCASIONES — el resto de ocasiones sigue por EnlacePrototipo).',
        states: [
          {
            label: 'Hero compacto congelado',
            kind: 'estado',
          ...
          },
          {
            label: 'Galería abierta (lightbox)',
            kind: 'overlay',
            to: '/eventos/bodas?dev-hero-galeria=abierta',
            note: 'El lightbox del mosaico incrustado en el hero (evento.galeria) — mismo flag y mismo componente (internas/galeria-hero.tsx) que la ficha de tour.',
          },
          {
